/* PetaPoco v3.0.1 - A Tiny ORMish thing for your POCO's.
...
namespace PetaPoco
{
    ...
    public class Database : IDisposable, IDatabase
    {
		...
		{
            get { return FormatLastCommand(_lastSql, _lastArgs); }
		}

        protected string FormatCommand(IDbCommand cmd) 
        {
            return FormatLastCommand(cmd.CommandText, (from IDataParameter parameter in cmd.Parameters select parameter.Value).ToArray());
        }

        private string FormatLastCommand(string sql, object[] args) 
        {
            var sb = new StringBuilder();
            if (sql == null)
                return "";
